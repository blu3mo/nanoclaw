...
- **north (タスク管理)**: `mcp__north__*` ツール群でユーザーの north のタスクを読み書きできる。kanban.md と並行して使うが、north はユーザーが他デバイスからも使えるのが利点。OAuth 認証済みでないと使えない（401 エラーが出る場合は認証が必要）
...
4. **チェックインを schedule_task で登録**: 宣言した終了時刻に `once` タスクを入れる。プロンプトは「[タスク名]の終了チェック。何ができた？」。`timezone` に USER.md のタイムゾーンを渡す（システムが自動変換する）
...
**自分で時刻変換をしてはいけない。** `schedule_task` の `timezone` パラメータに USER.md に記録されているタイムゾーン（IANA 形式、例: `Asia/Tokyo`）を渡す。cron 式や once の時刻はユーザーのローカル時間でそのまま書く。システムが自動変換する。

例: 日本のユーザーの朝8時 → `schedule_value: "0 8 * * *"`, `timezone: "Asia/Tokyo"` — これだけ。

...
3. kanban.md, USER.md, errands.md の初期化
4. ユーザーの基本情報ヒアリング → USER.md に記録
5. **タイムゾーンを確認** → USER.md に `タイムゾーン: America/New_York` 等の形式で記録。以降の全てのスケジュール登録でこのタイムゾーンを使う
6. 以下のスケジュールタスクを登録（cron 時刻はユーザーのタイムゾーンに合わせて調整する。サーバー TZ と異なる場合は変換が必要）:
   - 朝のセッション（cron、ユーザーの朝8:00、context_mode: group）
   - 夜のセッション（cron、ユーザーの夜23:00、context_mode: group）
   - **深夜フラッシュ**（cron、ユーザーの深夜3:00、context_mode: group、下記参照）
